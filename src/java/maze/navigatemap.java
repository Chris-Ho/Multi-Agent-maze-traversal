...
public class navigatemap extends DefaultInternalAction {
		...
		public synchronized Object execute(TransitionSystem ts,
			...
			for(int i =0;i<path.size();i++) {				
				//Marking Everything that agent is moving to as obstacle to prevent collision
...
//					}
					...
					if(map[agentmap.adjustX(adjacentNode[0])][agentmap.adjustY(adjacentNode[1])] != -1
							&& map[agentmap.adjustX(adjacentNode[0])][agentmap.adjustY(adjacentNode[1])]!=0
							&& map[agentmap.adjustX(adjacentNode[0])][agentmap.adjustY(adjacentNode[1])]!=5) { 
