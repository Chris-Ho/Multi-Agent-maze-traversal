...
+!scan : true
				<- .broadcast(tell, scanning(runner2));
					.wait(not motion(_));
					.broadcast(tell, scanning(runner2));
					.wait(not scanning(runner1));
					scan(1);
...
+!pathing : true 
			<-  .broadcast(tell, motion(runner2));
				.wait(not motion(runner1));
				?currentPath(MyPath);
				?x(X);
				?y(Y);
				.nth(0,MyPath, Dx);
				...
				.broadcast(tell, occupied(NewX, NewY, runner2));
...
+occupied([X,Y,N])[source(Other)] : true
				<-  .abolish(occupied([Q,C,_])[source(Other)] & Q\==X & C\==Y);
...
+obstructed(X1,Y1,X2,Y2) : true
					<- .findall([X,Y,Name], occupied([X,Y, Name]), AgentLocations);
						?x(MyX);
						?y(MyY);
						?currentPath(MyPath);
						.nth(0, MyPath, PathX);
						.nth(1, MyPath, PathY);
						.length(AgentLocations, N);
						-+counter(0);
						
						while(counter(Counter) & Counter<N){
							.nth(Counter, AgentLocations, CurrentAgent);
							.nth(0, CurrentAgent, TheirX);
							.nth(1, CurrentAgent, TheirY);
							.nth(2, CurrentAgent, NameOfAgent);
							if(MyX + X2 == TheirX & MyY + Y2 == TheirY){
								.send(NameOfAgent, achieve, sideStep(PathX, PathY));
							}
						}
						.
...
+currentPath(MyPath)[source(Other)] : Other \== self & Other \== "runner2"
				<-  .nth(0, MyPath, X);
					.nth(1,MyPath, Y);
					maze.removePath(X,Y);
					-+onPath(true);
...
@busy[atomic]
+busy[source(Other)] : Other \== "runner2"
		<- ?responses(R);
			-+responses(R+1);
			-busy.
