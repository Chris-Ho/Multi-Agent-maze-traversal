...
+!scan : true
				<- .broadcast(tell, scanning(runner4));
					.wait(not motion(_));
					.wait(not scanning(runner3) & not scanning(runner2) & not scanning(runner1));
					scan(1);
...
+!pathing : true 
			<-  .broadcast(tell, motion(runner4));
				.wait(not motion(runner2) & not motion(runner1) & not motion(runner3));
				?currentPath(MyPath);
				?x(X);
				?y(Y);
				.nth(0,MyPath, Dx);
...
+currentPath(MyPath)[source(Other)] : Other \== self & Other \== "runner4"
				<-  
					.nth(0, MyPath, X);
					.nth(1,MyPath, Y);
					maze.removePath(X,Y);
...
@busy[atomic]
+busy[source(Other)] : Other \== "runner4"
		<- ?responses(R);
			-+responses(R+1);
			-busy.
