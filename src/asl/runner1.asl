...
+!scan : true
				<- 	.broadcast(tell, scanning(runner1));
					.wait(not motion(_));
					scan(1);
...
+currentPath(MyPath)[source(Other)] : Other \== self & Other \== "runner1"
				<-  .print("given new path");
					.nth(0, MyPath, X);
					.nth(1,MyPath, Y);
					maze.removePath(X,Y);
					-+onPath(true);
...
@busy[atomic]
+busy[source(Other)] : Other \== "runner1"
		<- ?responses(R);
			-+responses(R+1);
			-busy.
