...
+!scan : true
				<- .broadcast(tell, scanning(runner3));
					.wait(not motion(_));
					.wait(not scanning(runner2) & not scanning(runner1));
					scan(1);
...
+!pathing : true 
			<-  .broadcast(tell, motion(runner3));
				.wait(not motion(runner2) & not motion(runner1));
				?currentPath(MyPath);
				?x(X);
				?y(Y);
				.nth(0,MyPath, Dx);
...
+currentPath(MyPath)[source(Other)] : Other \== self & Other \== "runner3"
				<-  .nth(0, MyPath, X);
					.nth(1,MyPath, Y);
					maze.removePath(X,Y);
					-+onPath(true);
...
@busy[atomic]
+busy[source(Other)] : Other \== "runner3"
		<- ?responses(R);
			-+responses(R+1);
			-busy.
